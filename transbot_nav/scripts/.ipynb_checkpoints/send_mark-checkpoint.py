...
class Multipoint_navigation:
    def __init__(self):
        ...
        self.pub_mark = rospy.Publisher('/robot1/path_point', MarkerArray, queue_size=100)
        # 订阅rviz内标记按下的位置 || Subscribe to mark the pressed position in rviz
        self.sub_click = rospy.Subscriber('/robot1/clicked_point', PointStamped, self.press_callback)
        # 发布目标点 || Publish target point
        self.pub_goal = rospy.Publisher('/robot1/move_base_simple/goal', PoseStamped, queue_size=1)
        # 取消目标点 || cancel target point
        self.pub_cancelgoal = rospy.Publisher("/robot1/move_base/cancel", GoalID, queue_size=10)
        # 订阅到达目标点的状态 || Subscribe to the status of reaching the target point
        self.sub_goal_result = rospy.Subscriber('/robot1/move_base/result', MoveBaseActionResult, self.goal_result_callback)
        # 订阅初始化位置话题 || Subscribe to the initial pose topic
        self.sub_initialpose = rospy.Subscriber('/robot1/initialpose', PoseWithCovarianceStamped, self.initialpose_callback)
