...
bool WaypointServer::RunWp(waypoints::Run_Wp::Request &req, waypoints::Run_Wp::Response &res)
{
  ...
  do
  {
	...
	ROS_INFO("Before condition check");
        if (ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
        {
	  ROS_INFO("Inside condition (Goal success)");
          ROS_INFO("Hello Grant");
	  ROS_INFO("Goal success");
	  ROS_INFO("Hello Dan");          
	  flag_success = "true";

	  // calls python script python ros-script.py with i as argument
	  // Convert integer to string
	  std::string string2 = std::to_string(i+1);

	  // Construct the command
	  std::string command = "dbus-launch gnome-terminal -- python3 /home/pi/Temperature-Detection/Python-Server/ros-script.py " + string2;

	  // Execute the command
	  int result = system(command.c_str());

	  if (result == -1) {
    		ROS_ERROR("Failed to execute the command");
	  } else {
    		ROS_INFO("Command executed with result %d", result);
	  }	
	...
	ROS_INFO("After condition check");
